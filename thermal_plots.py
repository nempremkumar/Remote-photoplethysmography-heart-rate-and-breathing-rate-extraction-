...
#temporal_signals['FFT'], transformed_signals['FFT'], filtered_signals['FFT'], heart_rates['FFT'] = extractor.method_fft()
#temporal_signals['PCA'], transformed_signals['PCA'], filtered_signals['PCA'], heart_rates['PCA'] = extractor.method_pca()
#temporal_signals['ICA'], transformed_signals['ICA'], filtered_signals['ICA'], heart_rates['ICA'] = extractor.method_ica()

# Saving combined plots
#extractor.save_combined_plot(temporal_signals, transformed_signals, filtered_signals, heart_rates, output_h5_file)

# Using FFT
#temporal_signals['FFT'], transformed_signals['FFT'], filtered_signals['FFT'], heart_rates['FFT'] = extractor.method_fft()
#print(f"Estimated Heart Rate using FFT: {heart_rates['FFT']:.2f} BPM")

# Using PCA
#temporal_signals['PCA'], transformed_signals['PCA'], filtered_signals['PCA'], heart_rates['PCA'] = extractor.method_pca()
#print(f"Estimated Heart Rate using PCA: {heart_rates['PCA']:.2f} BPM")

# Using ICA
#temporal_signals['ICA'], transformed_signals['ICA'], filtered_signals['ICA'], heart_rates['ICA'] = extractor.method_ica()
#print(f"Estimated Heart Rate using ICA: {heart_rates['ICA']:.2f} BPM")